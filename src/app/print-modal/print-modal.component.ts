...
export class PrintModalComponent {
  ...
  variantWithSizeRing(v?: string): SafeHtml {
    ...
    const html = v.replace(
      re,
      m => `<span class="size-ring size-ring--lg">${m.toUpperCase()}</span>`
    );
  ...
  <style>
    ...
    /* ---------- PACKING text bigger (paper only) ---------- */
    .packing-wrapper .items-table td .prod > div{
      font-size: 26px !important;
      line-height: 1.35 !important;
      font-weight: 800 !important;
    }
    .packing-wrapper .items-table td .prod > div .muted{
      font-size: 20px !important;
      font-weight: 600 !important;
    }
      .packing-wrapper .items-table .color-ar-en,
    .packing-wrapper .items-table .fabric-ar {
      font-size: 22px !important;
      font-weight: 700 !important;
    }
    .packing-wrapper .items-table .muted .color-ar-en,
    .packing-wrapper .items-table .muted .fabric-ar {
      font-size: 22px !important;
      font-weight: 700 !important;
    }
      .packing-wrapper .size-ring,
    .size-ring.size-ring--lg {
    ...
    /* Packing bigger thumbs + fit image */
  ...
  private buildPrintCSS(): string {
    ...
    <style>
      ...
      /* ---------- PACKING text bigger (bundle paper) ---------- */
      .packing-wrapper .items-table td .prod > div{
        font-size: 26px !important;
        line-height: 1.35 !important;
        font-weight: 800 !important;
      ...
      /* === Badge confined to packing page === */
      ...
      /* Never show badge on invoice pages */
      .invoice-wrapper .zone-badge { display:none !important; }
  
      /* Packing: bigger thumbs + fit image */
      .packing-wrapper .items-table .thumb{ width:100px !important; height:100px !important; }
      .packing-wrapper .thumb {
        object-fit: contain !important;
        object-position: center !important;
        background: #fff !important;
      }
  ...
  private async captureHTML(mode: 'invoice' | 'packing'): Promise<string> {
    ...
    if (mode === 'packing') {
      html = this.applyColorsArabicBilingual(this.applyFabricArabicOnly(html));
    }
    return html;
